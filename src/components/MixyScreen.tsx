...
const MixyScreen = ({
    ...
    const addCandidate = (candidate: MixySearchCandidate) => {
        const track = buildFederatedMixyTrack(candidate, candidates, mixy.deviceId, mixy.participantName);
        void mixy.addTrack(track);
    };
                        ...
                        <div className="mt-3 max-h-[350px] space-y-2 overflow-y-auto pr-1">
                            {groupedCandidates.map((candidate) => (
                                <div key={`${candidate.provider}:${candidate.id}`} className="grid grid-cols-[44px_minmax(0,1fr)_auto] items-center gap-3 border border-border p-2">
                                    ...
                                    <button type="button" onClick={() => addCandidate(candidate)} className="border border-primary p-2 text-primary" aria-label={`Agregar ${candidate.title}`}><Plus size={15} /></button>
                                </div>
                            ))}
